...
function resolveTenant(hostname: string) {
  if (hostname.endsWith('.platform.com')) {
    const slug = hostname.replace('.platform.com', '');
...
```txt
https://arik.platform.com/sitemap.xml
...
```txt
preview.platform.com/[preview-token]
```
...
- `username.platform.com` render site yang benar.
